...
if (isset($_GET["txtID"])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM `tbl_peliculas` WHERE id=:id");
...
if ($_POST) {
    ...
    $sentencia = $conexion->prepare("UPDATE `tbl_peliculas` 
        SET 
    nombre=:nombre, 
    foto=:foto,
    puntaje=:puntaje, 
    ...
    $sentencia->bindValue(":id", $txtID);
    $sentencia->execute();
    header("Location:index.php?mensaje=Pelicula editada");
}
